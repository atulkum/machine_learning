  ...
  def loss(self, X, y=None):
    ...
    for i in range(1,num_layers):
        if self.use_batchnorm:
            ...
            out, caches[i] = affine_relu_forward(x=out, w=self.params['W'+str(i)], 
                                                 b=self.params['b'+str(i)])
    ...
    out, caches[num_layers] = affine_forward(x=out, w=self.params['W'+str(num_layers)], 
    ...
    # of 0.5 to simplify the expression for the gradient.                      #
    ############################################################################
    loss, da = softmax_loss(x=scores, y=y)
    loss += 0.5*self.reg*np.sum(self.params['W'+str(num_layers)]*self.params['W'+str(num_layers)])

    da,dw,db = affine_backward(dout=da, cache=caches[num_layers])
    ...
    for i in range(num_layers-1,0,-1):
        ...
        if self.use_batchnorm:
            da, dw, db, dgamma, dbeta = affine_batchnorm_relu_backward(dout=da, cache=caches[i])
            grads['gamma'+str(i)] = dgamma
            grads['beta'+str(i)] = dbeta
        else:
            da, dw, db = affine_relu_backward(dout=da, cache=caches[i])
        grads['b'+str(i)] = db
