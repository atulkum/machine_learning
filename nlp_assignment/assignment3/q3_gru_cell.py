...
class GRUCell(tf.contrib.rnn.RNNCell):
    ...
    def __call__(self, inputs, state, scope=None):
        ...
        with tf.variable_scope(scope):
            ...
            b_z = tf.get_variable('b_z', shape=[self.state_size], initializer=tf.constant_initializer(1), dtype=tf.float32)
            b_r = tf.get_variable('b_r', shape=[self.state_size], initializer=tf.constant_initializer(1), dtype=tf.float32)
            b_o = tf.get_variable('b_o', shape=[self.state_size], initializer=tf.constant_initializer(1), dtype=tf.float32)
            ...
            o_t = tf.multiply(r_t, tf.matmul(state, W_o))
            o_t = tf.nn.tanh(tf.matmul(inputs, U_o) + o_t + b_o)
