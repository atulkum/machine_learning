...
class RNNModel(NERModel):
    ...
    def add_placeholders(self):
        ...
        """
        ### YOUR CODE HERE (~4-6 lines)
        self.input_placeholder = tf.placeholder(tf.int32, shape=[None, Config.max_length, self.config.n_features])
    ...
    def add_embedding(self):
        ...
        """
        ### YOUR CODE HERE (~4-6 lines)
        emb_W = tf.get_variable(name="emb_W", shape=self.pretrained_embeddings.shape,
                initializer=tf.constant_initializer(self.pretrained_embeddings), trainable=True)
        embeddings = tf.nn.embedding_lookup(emb_W, self.input_placeholder)
    ...
    def add_prediction_op(self):
        ...
        b2 = tf.Variable(tf.ones([Config.n_classes]))
        rnn_batch_size = tf.shape(x)[0]
        h = tf.zeros([rnn_batch_size, Config.hidden_size], dtype=tf.float32)
        ### END YOUR CODE
        ...
        with tf.variable_scope("RNN"):
            for time_step in range(self.max_length):
                ...
                h, c = cell(x[:, time_step, :], h)
                h_drop = tf.nn.dropout(h, keep_prob=dropout_rate)
                pred_t = tf.matmul(h_drop, U) + b2
                preds.append(pred_t)
                ### END YOUR CODE
    ...
    def add_loss_op(self, preds):
        ...
        '''
        sequence_lengths = tf.reduce_sum(tf.cast(self.mask_placeholder, tf.int32), axis =1)
        log_likelihood, transition_params = tf.contrib.crf.crf_log_likelihood(
            preds, self.labels_placeholder, sequence_lengths)
    ...
    def predict_on_batch(self, sess, inputs_batch, mask_batch):
        ...
        sequence_lengths = tf.reduce_sum(tf.cast(self.mask_placeholder, tf.int32), axis =1)
        viterbi_sequence, viterbi_score = tf.contrib.crf.crf_decode(self.pred, self.transition_params, sequence_lengths)
        tf_viterbi_sequence, tf_viterbi_score = sess.run([viterbi_sequence, viterbi_score], feed_dict=feed)
        predictions = tf_viterbi_sequence
